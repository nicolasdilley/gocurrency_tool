...
type PageData struct {
	...
	Project_counter               analyse.Counter
...
func GenerateProjectCounter(project_counter analyse.Counter) analyse.Counter {
...
func HtmlOutputCounters(package_counters []*PackageCounter, commit string, project_name string, index_data *IndexFileData, path_to_dir string) analyse.Counter {
	var filename string = "./" + Html_results_dir + "/" + strings.Replace(project_name, "/", "-", -1) + ".html"
	var project_counter analyse.Counter = analyse.Counter{Project_name: project_name}
...
// returns all the go files in the path_to_dir
func GeneratePackageListFiles(path_to_dir string) string {
	...
	if err != nil {
		...
		if !strings.HasSuffix(path_to_dir, "_test") {
			fmt.Fprintln(os.Stderr, "Error while running ls : ", err)
		}
	}
	filenames := ""
...
// Generate a list of all the files (inter-pro) in path_to_dir
func GenerateListFiles(path_to_dir string) string {
	...
	if err != nil {
		// if contains _test then the ls is not working correctly
		if strings.HasSuffix(path_to_dir, "_test") {
			// old path = ../x_text => new = ../x/x_test.go

			git_out = bytes.Buffer{}
			git_cmd.Stdout = &git_out
			err := git_cmd.Run()

			if err != nil {
				fmt.Fprintln(os.Stderr, "Error while running git ls-files : ", err)
			}
		} else {
			fmt.Fprintln(os.Stderr, "Error while running git ls-files : ", err)
		}
